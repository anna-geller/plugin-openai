...
@Schema(
    ...
    description = "For more informations, refer to the [Chat Completions API docs](https://platform.openai.com/docs/guides/gpt/chat-completions-api)"
)
@Plugin(
    examples = {
        @Example(
...
public class ChatCompletion extends AbstractTask implements RunnableTask<ChatCompletion.Output> {
    ...
    public ChatCompletion.Output run(RunContext runContext) throws Exception {
        ...
        if (this.messages == null && this.prompt == null) {
            throw new Exception("Either `messages` or `prompt` must be set");
        }
